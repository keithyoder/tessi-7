...
RSpec.describe Faturas::AtualizarValorService, type: :service do
  ...
  describe '.call' do
    context 'quando a fatura não está registrada' do
      ...
      it 'gera um novo nossonumero' do
        nova_fatura = described_class.call(fatura: fatura, novo_valor: 150.00)

        expect(nova_fatura.nossonumero).not_to eq(fatura.nossonumero)
        expect(nova_fatura.nossonumero).to be_present
      end

      it 'mantém o mesmo pagamento_perfil' do
        nova_fatura = described_class.call(fatura: fatura, novo_valor: 150.00)

        expect(nova_fatura.pagamento_perfil).to eq(fatura.pagamento_perfil)
      end
    end
    ...
    context 'quando ocorre um erro' do
      ...
      it 'faz rollback da transação' do
    ...
    context 'com valores decimais' do
      ...
      it 'mantém a precisão decimal' do
        nova_fatura = described_class.call(fatura: fatura, novo_valor: 239.70)

        expect(nova_fatura.valor.to_f).to eq(239.70)
        expect(nova_fatura.valor_original.to_f).to eq(99.99) # Mantém o valor original
      end
    end
    ...
    context 'quando vencimento_original está presente' do
      ...
      it 'preserva o vencimento_original' do
        nova_fatura = described_class.call(fatura: fatura, novo_valor: 150.00)
